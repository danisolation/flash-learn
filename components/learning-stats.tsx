...
export default function LearningStats() {
  ...
  const totalKnownCards = decks.reduce(
    (sum, deck) => sum + deck.cards.filter((card) => card.status === "known").length,
    0,
  )
  const overallProgress = totalCards > 0 ? Math.round((totalKnownCards / totalCards) * 100) : 0
    ...
    return (
      <Card>
        <CardContent className="p-6 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">Hãy bắt đầu học để xem thống kê</p>
        </CardContent>
      </Card>
    )
  }
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg text-center">
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{totalKnownCards}</p>
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg text-center">
              <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{overallProgress}%</p>
