...
import { ArrowLeft, Trophy, Clock, X, RotateCcw } from "lucide-react"
...
export default function GamePage() {
  ...
  const [gameMode, setGameMode] = useState<"select" | "match" | "speed" | "complete">("select")
          ...
          {decks.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {decks.map(deck => (
                <Card 
                  key={deck.id} 
                  className={`cursor-pointer transition-all ${selectedDeck === deck.id ? 'ring-2 ring-primary' : ''}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Ghép từ vựng với nghĩa tương ứng</CardDescription>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground mb-4">
                    Lật các thẻ để tìm cặp từ vựng và nghĩa tương ứng. Hoàn thành càng nhanh, điểm càng cao.
                  </p>
                </CardContent>
                <CardFooter>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground mb-4">
                    Xem từ vựng và chọn nhanh xem bạn đã thuộc hay chưa. Trả lời đúng để ghi điểm.
                  </p>
                </CardContent>
                <CardFooter>
              ...
              >
                <div 
                  className={`relative h-24 md:h-32 w-full cursor-pointer transition-transform duration-500 transform-style-3d ${
                    flippedCards.includes(index) || matchedPairs.includes(card.pairId) ? 'rotate-y-180' : ''
                  }`}
                >
      ...
      {gameMode === "speed" && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-muted-foreground" />
              <span className={`font-bold ${speedGameTime <= 10 ? 'text-red-500' : 'text-muted-foreground'}`}>
                {speedGameTime}s
              </span>
            </div>
          </div>
                  ...
                  <Card className="w-full shadow-lg">
                    <CardContent className="p-6 flex flex-col items-center">
                      ...
                      <div className="text-center mb-4">
                        <p className="text-sm text-muted-foreground">Bạn có biết nghĩa của từ này không?</p>
                      </div>
                    </CardContent>
